...
document.addEventListener("DOMContentLoaded", function () {
  ...
  moneyInputs.forEach((inputId) => {
    ...
    input.addEventListener("input", () =>
      window.SimulacaoFinanceiraUtils.applyMoneyMask(input),
    );
  ...
  function displayResults(results) {
    ...
    // Descontos opcionais
    const optionalSection = document.getElementById(
      "optionalDeductionsSection",
    );
  ...
  salaryForm.addEventListener("submit", function (e) {
    ...
    const formData = {
      grossSalary: window.SimulacaoFinanceiraUtils.parseCurrency(
        document.getElementById("grossSalary").value,
      ),
      dependents: parseInt(document.getElementById("dependents").value) || 0,
      healthPlan: window.SimulacaoFinanceiraUtils.parseCurrency(
        document.getElementById("healthPlan").value,
      ),
      mealVoucher: window.SimulacaoFinanceiraUtils.parseCurrency(
        document.getElementById("mealVoucher").value,
      ),
      transportVoucher: window.SimulacaoFinanceiraUtils.parseCurrency(
        document.getElementById("transportVoucher").value,
      ),
      otherDeductions: window.SimulacaoFinanceiraUtils.parseCurrency(
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { calculateINSS, inssTable, calculateNetSalary };
}
