...
const CONSTANTS = {
  ...
  MULTA_FGTS_DEMISSAO: 0.40, // 40%
  MULTA_FGTS_ACORDO: 0.20, // 20%
  MULTA_FGTS_SAQUE_ANIVERSARIO: 0.20, // 20% para optantes do saque-aniversário
...
function setupEventListeners() {
  ...
  const formInputs = [
    'salario', 'dataAdmissao', 'dataRescisao', 'tipoRescisao',
    'diasAviso', 'feriasVencidas', 'saqueAniversario', 'saldoFGTS'
  ];
...
function updateFormBasedOnRescisionType() {
  ...
  const saqueAniversarioContainer = document.getElementById('saqueAniversario').parentElement.parentElement;
...
function calculateRescision() {
  try {
    ...
    if (dados.salario < CONSTANTS.SALARIO_MINIMO_2025) {
      if (!confirm(`O salário informado (${formatMoney(dados.salario)}) está abaixo do salário mínimo (${formatMoney(CONSTANTS.SALARIO_MINIMO_2025)}). Deseja continuar?`)) {
        return;
      }
    }
    
    // Calcula período trabalhado
    const periodoTrabalhado = calculateWorkPeriod(dados.dataAdmissao, dados.dataRescisao);
...
function collectFormData() {
  return {
    ...
    feriasVencidas: parseFloat(document.getElementById('feriasVencidas').value) || 0,
    saqueAniversario: document.getElementById('saqueAniversario').checked,
    saldoFGTS: parseMoneyToFloat(document.getElementById('saldoFGTS').value) || 0
  };
}
...
function calculateWorkPeriod(dataAdmissao, dataRescisao) {
  ...
  while (tempDate.getFullYear() < dataRescisao.getFullYear() || 
  ...
  while (tempDate.getMonth() < dataRescisao.getMonth() || 
         (tempDate.getMonth() === dataRescisao.getMonth() && 
          tempDate.getDate() <= dataRescisao.getDate())) {
...
function calculateSalaryBalance(dados) {
  const diasNoMes = new Date(dados.dataRescisao.getFullYear(), dados.dataRescisao.getMonth() + 1, 0).getDate();
...
function calculateProportionalVacation(dados, periodo) {
  ...
  // Considera 15 dias ou mais como mês completo
  const diasRestantes = Math.floor((diffTime % (1000 * 60 * 60 * 24 * 30)) / (1000 * 60 * 60 * 24));
...
function calculateFGTSPenalty(dados) {
  ...
  if (dados.tipoRescisao === 'demissao-justa-causa' || dados.tipoRescisao === 'pedido-demissao') {
    return 0;
  }
  ...
  if (dados.saqueAniversario && dados.tipoRescisao === 'demissao-sem-justa-causa') {
...
function displayResults(verbas, periodo, dados) {
  ...
  document.getElementById('saldoSalario').textContent = formatMoney(verbas.saldoSalario);
  document.getElementById('avisoPrevio').textContent = formatMoney(verbas.avisoPrevio);
  document.getElementById('decimoTerceiro').textContent = formatMoney(verbas.decimoTerceiro);
  document.getElementById('feriasVencidasValor').textContent = formatMoney(verbas.feriasVencidas);
  document.getElementById('feriasProporcionais').textContent = formatMoney(verbas.feriasProporcionais);
  document.getElementById('multaFGTS').textContent = formatMoney(verbas.multaFGTS);
...
function updateObservations(dados) {
  ...
  // Observações gerais
  observacoesList.innerHTML += '<li>Os valores são brutos, antes dos descontos de INSS e IRRF</li>';
  observacoesList.innerHTML += '<li>FGTS será depositado na conta vinculada</li>';
  ...
  switch (dados.tipoRescisao) {
    case 'demissao-sem-justa-causa':
      ...
      observacoesList.innerHTML += '<li>Liberação total do FGTS + multa de 40%</li>';
      if (dados.saqueAniversario) {
        observacoesList.innerHTML += '<li>Como optante do saque-aniversário, multa reduzida para 20%</li>';
      }
      break;
    ...
    case 'demissao-justa-causa':
      ...
      observacoesList.innerHTML += '<li>Direito apenas ao saldo de salário e férias vencidas (se houver)</li>';
      break;
  ...
  if (dados.feriasVencidas > 0) {
    observacoesList.innerHTML += `<li>Consideradas ${dados.feriasVencidas.toFixed(1)} período(s) de férias vencidas</li>`;
  }
}
...
function updateCurrentDate() {
  ...
  document.getElementById('currentDate').textContent = `Atualizado em: ${dateString}`;
}
